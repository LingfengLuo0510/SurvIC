...
load("N5k_single_constant_2021.RData")
...
while (loop<nloop) {
  ...
  n_f = rep(10000, F)#rpois(F, lambda = 1000)  #sample size for each facility
  N=sum(n_f)
  ...
  for(lambda_index in 1:length(lambda_all)){
    ...
    model1 <- surtiver(fmla, data_NR, nsplines=K, spline ="Smooth-spline", ties="none", tau=0.5, stop="ratch",
                       method = "Newton", btr = "dynamic", iter.max = 15, threads = 4, parallel = TRUE,
...
save.image(file = "N5000_p1_constant_smoothcubic_startfrom1_500times_includeHastie.RData")
