...
while (loop<nloop) {
  ...
  for(lambda_index in 1:length(lambda_all)){
    ...
    model1 <- surtiver(fmla, data_NR, nsplines=K, spline ="Smooth-spline", ties="none", tau=0.5, stop="ratch",
                       method = method_tmp, btr = btr_tmp, iter.max = 15, threads = 4, parallel = TRUE,
...
save.image(file = "N1000_p1_constant_smoothcubic_from1_500times_includeHastie.RData")
